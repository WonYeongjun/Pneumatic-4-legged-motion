...
int ang_in =130, ang_out =0, ang_init =65;
...
int pos;
int p11 =3, p21 =6, p31 =9;
int sp1 =255, sp2 =0;
float minsp =150; // 팽창하는 최소 유량
float state[3];
...
void angle_to_rel_state(float* state, double angle, float length) {
  ...
  if (30 <= angle && angle < 150) { // A : 1 , 2
    ...
    state[0] = 0;
    state[1] = low * length *(255-minsp)/255 + minsp;
    state[2] = high* length *(255-minsp)/255 + minsp;
  } 
    ...
    high = high_tmp / (low_tmp + high_tmp);
    state[1] = 0;
    state[2] = low * length *(255-minsp)/255 + minsp;
    state[0] = high* length *(255-minsp)/255 + minsp;
  } 
  else { // C : 0 , 1
    ...
    high = high_tmp / (low_tmp + high_tmp);
    state[2] = 0;
    state[0] = low * length *(255-minsp)/255 + minsp;
    state[1] = high* length *(255-minsp)/255 + minsp;
    }
  Serial.print("state : ");
